...
struct AndromedaExtension {
    cached_binary_path: Option<String>,
}

impl AndromedaExtension {
    fn language_server_binary_path(
        &mut self,
        language_server_id: &LanguageServerId,
        worktree: &zed::Worktree,
    ) -> Result<String> {
        // First check if 'andromeda' is available in PATH
        if let Some(path) = worktree.which("andromeda") {
            return Ok(path);
        }
        ...
        let version_dir = format!("andromeda-{}", release.version);
        let binary_name = match platform {
            zed::Os::Windows => "andromeda.exe",
            _ => "andromeda",
        };
...
impl zed::Extension for AndromedaExtension {
    fn new() -> Self {
        Self {
    ...
    fn language_server_workspace_configuration(
        ...
        let settings = LspSettings::for_worktree("andromeda", worktree)
            .ok()
...
zed::register_extension!(AndromedaExtension);
