...
export function activate(context: vscode.ExtensionContext) {
    ...
    outputChannel = vscode.window.createOutputChannel('Andromeda Language Server');
    outputChannel.clear();
    ...
    try {
        ...
        vscode.window.showInformationMessage(
            ...
            if (selection === 'Show Commands') {
                vscode.commands.executeCommand('workbench.action.showCommands', 'Andromeda');
        ...
        outputChannel.show();
        vscode.window.showErrorMessage(`Andromeda extension failed to activate: ${error}`);
    }
}
...
function startLanguageServer(context: vscode.ExtensionContext) {
    ...
    if (!config.get('enable', true)) {
        ...
        vscode.window.showInformationMessage('Andromeda Language Server is disabled. Enable it in settings to use language features.');
        return;
    }
    ...
    if (isAbsolutePath && !fs.existsSync(executablePath)) {
        const message = `Andromeda executable not found at "${executablePath}". Please install Andromeda or update the path in settings.`;
        outputChannel.appendLine(`ERROR: ${message}`);
        vscode.window.showErrorMessage(message, 'Install Guide', 'Open Settings').then(selection => {
            if (selection === 'Install Guide') {
                vscode.env.openExternal(vscode.Uri.parse('https://github.com/tryandromeda/andromeda#installation'));
    ...
    if (!isAbsolutePath) {
        ...
        console.log(`Attempting to start Andromeda LSP server from PATH: ${executablePath}`);
        outputChannel.appendLine(`Attempting to start server from PATH: ${executablePath}`);
    }
    ...
    outputChannel.appendLine(`Starting LSP server with command: ${executablePath} lsp`);
    ...
    const clientOptions: LanguageClientOptions = {
        ...
        synchronize: {
            ...
            fileEvents: [
                ...
                vscode.workspace.createFileSystemWatcher('**/andromeda.config.{js,json,toml,yaml}'),
                vscode.workspace.createFileSystemWatcher('**/andromeda.{json,toml,yaml}')
            ]
        },
    ...
    client.start().then(() => {
        ...
        outputChannel?.appendLine('✓ Andromeda language server started successfully');
        outputChannel?.appendLine('Ready to provide JavaScript and TypeScript language features');
        ...
        let errorMessage = `Failed to start Andromeda Language Server: ${error.message}`;
        let actions: string[] = ['Show Output'];
        
        if (error.message.includes('ENOENT') || error.message.includes('not found')) {
            errorMessage = `Andromeda executable not found. Please install Andromeda or check your 'andromeda.executablePath' setting.`;
            actions = ['Install Guide', 'Open Settings', 'Show Output'];
            outputChannel?.appendLine('Suggestion: Install Andromeda or check executable path setting');
        } else if (error.message.includes('already exists')) {
            errorMessage = `Command registration conflict detected. This usually resolves after restarting the extension.`;
...
function registerCommands(context: vscode.ExtensionContext) {
    ...
    const showOutputCommand = vscode.commands.registerCommand('andromeda.showOutput', () => {
        if (outputChannel) {
            ...
            vscode.window.showInformationMessage('Andromeda Language Server is not running');
        }
    });

    // Apply auto fix command
    const applyAutoFixCommand = vscode.commands.registerCommand('andromeda.client.applyAutoFix', async () => {
        if (!client) {
            vscode.window.showErrorMessage('Andromeda Language Server is not running');
            return;
        }
    ...
    // Fix all command
    const fixAllCommand = vscode.commands.registerCommand('andromeda.client.fixAll', async () => {
        if (!client) {
            vscode.window.showErrorMessage('Andromeda Language Server is not running');
            return;
        }
        ...
        try {
            ...
            if (result) {
                vscode.window.showInformationMessage('All auto-fixable problems fixed');
            }
        } catch (error) {
    ...
    const formatCommand = vscode.commands.registerCommand('andromeda.format', async () => {
        const activeEditor = vscode.window.activeTextEditor;
        if (!activeEditor) {
            vscode.window.showErrorMessage('No active editor');
            return;
